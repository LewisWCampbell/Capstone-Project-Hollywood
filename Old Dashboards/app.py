...
col2.metric("Pipeline Artifacts", "Available" if status['pkl_exists'] else "Not Found")
col3.metric("OMDB Data", "Available" if status['omdb_exists'] else "Not Found")

if not status['pkl_exists']:
    st.warning(
...
pages = [
    ...
    ("6. Rail Manager", "Manage content rails — rename, merge, split clusters, override assignments, and export mappings."),
    ("7. Audio Coverage", "Phase 2 gate — monitor YouTube trailer coverage before enabling audio features."),
]
