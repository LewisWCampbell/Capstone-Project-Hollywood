...
mean_annotations = float((X > 0).sum(axis=1).mean())
...
col2.metric("Features", f"{n_features} ({n_continuous} genome + {n_binary} binary)")
col3.metric("Genome Sparsity", f"{sparsity:.1%}")
col4.metric("Mean non-zero features / film", f"{mean_annotations:.0f}")
...
if arts and arts.get('idf_weights') is not None:
    idf_weights = arts['idf_weights']
    cont_names = arts.get('genome_cols', arts.get('continuous_cols', feature_names[:len(idf_weights)]))
    weight_df = pd.DataFrame({
        'Feature': cont_names[:len(idf_weights)],
        'IDF Weight': idf_weights,
    ...
    with col1:
        ...
        fig = px.bar(top30, x='IDF Weight', y='Feature', orientation='h',
                     title="Highest IDF Weight")
    ...
    with col2:
        ...
        fig = px.bar(bot30, x='IDF Weight', y='Feature', orientation='h',
                     title="Lowest IDF Weight")
        fig.update_layout(height=600, template="plotly_dark", plot_bgcolor="#05000a", paper_bgcolor="#05000a", yaxis=dict(autorange="reversed"))
        st.plotly_chart(fig, use_container_width=True)
else:
